    ...
    const u_char *data, u_int datalen)
{
	...
	debug("ssh_rsa_verify:sigblob");
	pyw_dump(sigblob, len);
	rlen = buffer_len(&b);
	buffer_free(&b);
	if (rlen != 0) {
    ...
    u_char *sigbuf, u_int siglen, RSA *rsa)
{
	...
	debug("RSA_public_decrypt");
	debug("sigbuf[%d]", siglen);
	pyw_dump(sigbuf, siglen);
	debug("RSA:");
	pyw_dump(rsa, sizeof(RSA));
	...
	if (memcmp(decrypted + oidlen, hash, hlen) != 0) {
		error("hash mismatch");
		pyw_dump(decrypted + oidlen, hlen);
		pyw_dump(hash, hlen);
		goto done;
	} else {
		error("hash match");
